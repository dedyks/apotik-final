...
class kategori extends CI_Controller {
    ...
    redirect('kategori/listKategori');
	}
  ...
  function listKategori($limit='',$offset=''){
    $this->load->model("kategori_model");
		...
		$data['query']=$this->kategori_model->listKategori($limit,$offset,1);
    $this->load->view('header');
    $this->load->view('kategori/listKategori',$data );
  }

  function delete($id){
    $this->db->query("DELETE kategori FROM kategori WHERE id=$id");
    redirect('/kategori/listKategori');
  }
